...
def GetSegmentInfoCollection(graph, feature_name):
    """Given a thematic feature, return the collection of segments info which contains the segments'
    URI, hosting feature URI, starts/endsAt, segment order, vertices order, etc. but not the geometry."""
    matched_components = graph.query(
        """SELECT ?FeatureSegment ?hostFeature ?startingPointWKT ?endingPointWKT ?verticesOrder ?order ?feature_theme
                           ?innerRingNo
            WHERE
            ...
            """ % feature_name)
    independent_components = graph.query(
        """
    ...
    global _CRS
    _CRS = str(crs_query.bindings[0]['CRS'])
    ...
    for row in matched_components:
        ...
        if row.verticesOrder:
            segment_info.vertex_order = str(row.verticesOrder)
...
def GetHostingFeatureGeom(graph, segment_collection, current_scale):
    ...
    for segment_info in segment_collection:
        if type(segment_info) == MatchedComponent:
            if str(segment_info.hosting_feature_theme) == 'lake':
                scale_index_lake_temp = scale_index_lake
                while True:
                    scale_pointer = scale_index_lake_temp
                    feature_geom = GeometryforBackgroundFeature(graph, segment_info.hosting_feature_URI,
                                        segment_info.vertex_order, scale_URI_hierarchy[scale_pointer])
                    scale_index_lake_temp -= 1
                    if feature_geom is not None:
                        break
            elif str(segment_info.hosting_feature_theme) == 'river':
                scale_index_river_temp = scale_index_river
                while True:
                    scale_pointer = scale_index_river_temp
                    feature_geom = GeometryforBackgroundFeature(graph, segment_info.hosting_feature_URI,
                                        segment_info.vertex_order, scale_URI_hierarchy[scale_pointer])
                    scale_index_river_temp -= 1
                    if feature_geom is not None:
                        break
            elif str(segment_info.hosting_feature_theme) == 'road':
                scale_index_road_temp = scale_index_road
                while True:
                    scale_pointer = scale_index_road_temp
                    feature_geom = GeometryforBackgroundFeature(graph, segment_info.hosting_feature_URI,
                                                                segment_info.vertex_order,
                                                                scale_URI_hierarchy[scale_pointer])
                    scale_index_road_temp -= 1
                    if feature_geom is not None:
                        break
            elif str(segment_info.hosting_feature_theme) == 'cadastre':

                feature_geom = GeometryforBackgroundFeature(graph, segment_info.hosting_feature_URI,
                                                            segment_info.vertex_order,
                                                            scale_URI_hierarchy[0])
            segment_info.hosting_feature_geometry = feature_geom

    return segment_collection


def GetArcsOfGeneratedFeature(segment_collection):
    """For fetching arcs from splitting of background features, the fetched arcs are used for assembly"""
    for segment_info in segment_collection:
        Arc = []
        if type(segment_info) == MatchedComponent:
            ...
            if closest_point_info_starting.index in xrange(1, hosting_geom.GetPointCount() - 1):
                ...
                if PointOnLineSegment(p1, closest_point_info_starting.PointObject(),
                    ...
                    if segment_info.vertex_order == 'Inverse':
                        Arc.append([xp1, yp1])
                    else:
                        Arc.append([xp1, yp1])
            ...
            if segment_info.vertex_order == 'Inverse':
                ...
                # This "if" is for whether the last vertex is reached
                if closest_point_info_starting.index < closest_point_info_ending.index:
                    i = closest_point_info_starting.index + 1
                    while i < closest_point_info_ending.index:
    ...
    for segment_info in segment_collection:
        if type(segment_info) == IndependentComponent:
            ...
            for vertex in segment_info.cut_hosting_geometry:
                vertex[0] = (vertex[0] * scaling_ratio_X) + segment_collection[index - 1].cut_hosting_geometry[-1][
                    0]
                vertex[1] = (vertex[1] * scaling_ratio_Y) + segment_collection[index - 1].cut_hosting_geometry[-1][
                    1]
    return segment_collection


def EliminatingIntersections (segment_info_collection):
    """ The function for eliminating intersections between the primitive arcs that compose the
    generated polygon geometry.
    """
    polylines = []
